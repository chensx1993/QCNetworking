...
@implementation NSDictionary (QCNetworking)

/** 字符串前面是没有问号的，如果用于POST，那就不用加问号，如果用于GET，就要加个问号 */
- (NSString *)QC_urlParamsStringSignature:(BOOL)isForSignature
{
    NSArray *sortedArray = [self QC_transformedUrlParamsArraySignature:isForSignature];
    return [sortedArray QC_paramsString];
}
